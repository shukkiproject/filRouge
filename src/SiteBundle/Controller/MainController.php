...
class MainController extends Controller
{
    ...
    /**
     * @Route("/language/{_locale}", requirements={"_locale": "en|fr"}, name="site_main_language")
     */
    public function languageAction($_locale){
        ...
        if (isset($url)) {
            //deal with the special case from the french homepage
            $fr = strpos($url, 'fr');
            $en = strpos($url, 'en');
            if ($fr===false && $en===false) {
                return new RedirectResponse($url.'en');
            }

        $newLocale = $_locale; 
        //old locale is the opposite of the new one  
        $oldLocale = ($newLocale==='en')? 'fr' : 'en';   
        //replace the old locale by the new one.     
